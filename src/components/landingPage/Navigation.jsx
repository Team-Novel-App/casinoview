...
import { Dice6, Menu, X, LogOut, User, Settings,CreditCard} from "lucide-react";
...
export default function Navigation({ isScrolled }) {
  ...
  return (
    <nav className={`fixed left-0 right-0 mx-auto z-50 transition-all  bg-random duration-300 ${isScrolled ? "w-full sm:px-4 py-5 lg:px-10 ":"w-[95%] border-white lg:rounded-full md:rounded-full sm:rounded-4xl mt-8 px-4 py-5"} `}>      
      <div className=" px-4 w-full mr-10">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <a href="#home" className="hover:text-[#4f46e5] text-black ">Home</a>
            <a href="#games" className="hover:text-[#4f46e5] text-black">Games</a>
            <a href="#tournaments" className="hover:text-[#4f46e5] text-black">Events</a>
            <a href="#rules" className="hover:text-[#4f46e5] text-black">Rules</a>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-4">
              {user && (
                <button className="hidden md:flex items-center gap-2 text-black bg-white px-4 py-1 font-medium transition-transform hover:scale-105 border border-custom1">
                <CreditCard size={18} />
                <span>Payment</span>
              </button>
            )}
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-white mt-4 p-4 rounded-lg">
            {user && (
              ...
              <p className="text-center font-semibold text-gray-800  px-4 py-2 ">Hello, {user?.name} </p>
                <div className="relative w-8 h-8">
                  <Image src={user?.avatar} alt="User" width={30} height={30} className="rounded-full border w-full h-full"/>
            ...
            <div className="flex flex-col gap-4 mx-7 my-4">
              <a href="#home" className="hover:text-game-primary text-black">Home</a>
              <a href="#games" className="hover:text-game-primary text-black">Games</a>
              <a href="#tournaments" className="hover:text-game-primary text-black">Events</a>
              <a href="#rules" className="hover:text-game-primary text-black">Rules</a>
